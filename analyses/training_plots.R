...
training_plots <- function(data){
  plot <- ggplot(data = data, 
    ...
    geom_boxplot(width = 0.2, alpha = 0.9, linewidth = 0.3, colour = "grey20", 
                 outlier.shape = NA) +
    ...
    facet_wrap(~train_type, labeller = labeller(train_type = c( # no facet wrap for transition entropy
      "stable" = "Stable",
      "variable" = "Variable"
    ))) +
    ...
    theme(
      ...
      strip.background = element_rect(fill = "#D3D4D8", colour = NA),
